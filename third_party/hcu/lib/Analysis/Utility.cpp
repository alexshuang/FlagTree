...
bool isMfmaToDotShortcut(RankedTensorType srcTy, RankedTensorType dstTy) {
  auto mfmaLayout = dyn_cast<HCUMfmaEncodingAttr>(srcTy.getEncoding());
  ...
  return mfmaLayout.getWarpsPerCTA()[1] == 1 &&
         ...
         dotOperandLayout.getKWidth() == getSizePerThread(mfmaLayout)[1] &&
         dstmfmaLayout && dstmfmaLayout.getInterleave() &&
         (mfmaLayout.getMDim() == 32 || mfmaLayout.getMDim() == 16) &&
         (srcTy.getElementType().isF16() || srcTy.getElementType().isBF16());
}

bool isMfmaXMfmaCvt(RankedTensorType srcTy, RankedTensorType dstTy) {
