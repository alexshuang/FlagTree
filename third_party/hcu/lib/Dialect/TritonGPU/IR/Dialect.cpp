...
SmallVector<unsigned> getCTAsPerCGA(Attribute layout) {
  ...
  else if (mlir::isa<HCUMfmaEncodingAttr, HCUWmmaEncodingAttr>(layout))
    return {1, 1};
  else if (auto sharedLayout = mlir::dyn_cast<SharedEncodingAttr>(layout))
...
unsigned getNumWarpsPerCTA(Attribute layout) {
  ...
  } else if (auto mfmaLayout = dyn_cast<HCUMfmaEncodingAttr>(layout))
    warpsPerCTA = mfmaLayout.getWarpsPerCTA();
  else if (auto wmmaLayout = dyn_cast<HCUWmmaEncodingAttr>(layout))
...
SmallVector<unsigned>
HCUMfmaEncodingAttr::getElemsPerThread(ArrayRef<int64_t> shape,
                                       Type eltTy) const {
  size_t rank = shape.size();
  ...
  if (!getIsTransposed()) {
    ...
    elemsPerThread[rank - 2] = elemsRow * betaRow;
    elemsPerThread[rank - 1] = elemsCol * betaCol;
  } else {
    unsigned elemsCol =
        ceil<unsigned>(shape[rank - 1], nDim * getWarpsPerCTA()[rank - 1]);
    unsigned elemsRow =
...
unsigned HCUMfmaEncodingAttr::getTotalElemsPerThread(ArrayRef<int64_t> shape,
                                                     Type eltTy) const {
  return product<unsigned>(getElemsPerThread(shape, eltTy));
}
...
SmallVector<unsigned>
HCUWmmaEncodingAttr::getElemsPerThread(ArrayRef<int64_t> shape,
                                       Type eltTy) const {
  size_t rank = shape.size();
...
unsigned HCUWmmaEncodingAttr::getTotalElemsPerThread(ArrayRef<int64_t> shape,
                                                     Type eltTy) const {
  return product<unsigned>(getElemsPerThread(shape, eltTy));
}
...
LogicalResult DotOperandEncodingAttr::verify(
  ...
  if (auto parentAttr = mlir::dyn_cast<HCUWmmaEncodingAttr>(parent)) {
  ...
  if (auto parentAttr = mlir::dyn_cast<HCUMfmaEncodingAttr>(parent)) {
    if (kWidth == 0)
      return emitError()
             << "triton_gpu.dot_op kWidth parameter is mandatory for "
                "MFMA parent";
...
Attribute HCUMfmaEncodingAttr::parse(AsmParser &parser, Type type) {
  if (parser.parseLess().failed())
    return {};
  DictionaryAttr dict;
  if (parser.parseAttribute(dict).failed())
  ...
  for (const NamedAttribute &attr : dict) {
    ...
    if (attr.getName() == "interleave") {
      if (parseBool(parser, attr, interleave, "interleave").failed())
        return {};
    }
    if (attr.getName() == "CTASplitNum") {
      if (parseIntArrayAttr(parser, attr, CTASplitNum.emplace(), "CTASplitNum")
  ...
  return parser.getChecked<HCUMfmaEncodingAttr>(
      parser.getContext(), versionMajor, versionMinor, warpsPerCTA,
      instrShape[0], instrShape[1], KDim, isTransposed, interleave, *CTALayout);
}

void HCUMfmaEncodingAttr::print(AsmPrinter &printer) const {
  printer << "<{"
          << "versionMajor = " << getVersionMajor()                      //
          << ", versionMinor = " << getVersionMinor()                    //
          << ", warpsPerCTA = [" << ArrayRef(getWarpsPerCTA()) << "]"    //
...
Attribute HCUWmmaEncodingAttr::parse(AsmParser &parser, Type type) {
  if (parser.parseLess().failed())
    return {};
  DictionaryAttr dict;
  if (parser.parseAttribute(dict).failed())
  ...
  return parser.getChecked<HCUWmmaEncodingAttr>(parser.getContext(), version,
                                                warpsPerCTA, *CTALayout);
}

void HCUWmmaEncodingAttr::print(AsmPrinter &printer) const {
  printer << "<{"
...
SmallVector<unsigned>
HCUMfmaEncodingAttr::getShapePerCTATile(ArrayRef<int64_t> tensorShape) const {
  auto warpsPerCTA = getWarpsPerCTA();
...
SmallVector<unsigned> HCUMfmaEncodingAttr::getCTAsPerCGA() const {
  return SmallVector<unsigned>(getCTALayout().getCTAsPerCGA());
}
SmallVector<unsigned> HCUMfmaEncodingAttr::getCTAOrder() const {
  return SmallVector<unsigned>(getCTALayout().getCTAOrder());
}
SmallVector<unsigned> HCUMfmaEncodingAttr::getCTASplitNum() const {
  return SmallVector<unsigned>(getCTALayout().getCTASplitNum());
}
SmallVector<unsigned> HCUMfmaEncodingAttr::getWarpsPerCTA() const {
  return SmallVector<unsigned>(getWarpsPerCTA__());
}
SmallVector<unsigned> HCUMfmaEncodingAttr::getWarpOrder() const {
  return ::getWarpOrder(*this);
}
SmallVector<unsigned> HCUMfmaEncodingAttr::getThreadOrder() const {
  return ::getOrder(*this);
}
SmallVector<unsigned> HCUMfmaEncodingAttr::getThreadsPerWarp() const {
  unsigned rows, cols;
...
SmallVector<unsigned> HCUMfmaEncodingAttr::getSizePerThread() const {
  unsigned rows, cols;
  auto rank = ::getOrder(*this).size();
  SmallVector<unsigned> res(rank, 1);
  if (getMDim() == 32 && getNDim() == 32) {
...
SmallVector<int64_t>
HCUMfmaEncodingAttr::getMFMAInstrShapeForOperands(int kWidth, int opIdx) const {
...
SmallVector<int64_t>
HCUMfmaEncodingAttr::getMFMARepForOperands(ArrayRef<int64_t> operandShape,
...
SmallVector<unsigned>
HCUMfmaEncodingAttr::getSizePerThreadForOperands(unsigned opIdx) const {
  if (opIdx == 0) {
    return {4, 1};
  } else if (opIdx == 1) {
    return {1, 4};
...
SmallVector<unsigned>
HCUMfmaEncodingAttr::getShapePerCTATileForDotOperands(ArrayRef<int64_t> shape,
                                                      int opIdx) const {
...
SmallVector<unsigned>
HCUWmmaEncodingAttr::getShapePerCTATile(ArrayRef<int64_t> tensorShape) const {
  auto warpsPerCTA = getWarpsPerCTA();
  auto rank = warpsPerCTA.size();
  SmallVector<unsigned> shapePerCTATile(warpsPerCTA.begin(), warpsPerCTA.end());
...
SmallVector<unsigned> HCUWmmaEncodingAttr::getCTAsPerCGA() const {
  return SmallVector<unsigned>(getCTALayout().getCTAsPerCGA());
}
SmallVector<unsigned> HCUWmmaEncodingAttr::getCTAOrder() const {
  return SmallVector<unsigned>(getCTALayout().getCTAOrder());
}
SmallVector<unsigned> HCUWmmaEncodingAttr::getCTASplitNum() const {
  return SmallVector<unsigned>(getCTALayout().getCTASplitNum());
}
SmallVector<unsigned> HCUWmmaEncodingAttr::getWarpsPerCTA() const {
  return SmallVector<unsigned>(getWarpsPerCTA__());
}
SmallVector<unsigned> HCUWmmaEncodingAttr::getWarpOrder() const {
  return ::getWarpOrder(*this);
}
SmallVector<unsigned> HCUWmmaEncodingAttr::getThreadOrder() const {
  return ::getOrder(*this);
}
SmallVector<unsigned> HCUWmmaEncodingAttr::getThreadsPerWarp() const {
...
SmallVector<unsigned> HCUWmmaEncodingAttr::getSizePerThread() const {
  auto rank = getWarpsPerCTA().size();
...
SmallVector<unsigned>
HCUWmmaEncodingAttr::getSizePerThreadForOperands(unsigned opIdx) const {
  auto rank = getWarpsPerCTA().size();
  SmallVector<unsigned> sizePerThread(rank, 1);
...
SmallVector<unsigned>
HCUWmmaEncodingAttr::getShapePerCTATileForDotOperands(ArrayRef<int64_t> shape,
                                                      int opIdx) const {
...
unsigned HCUWmmaEncodingAttr::getTotalElemsPerThreadForOperands(
    ArrayRef<int64_t> shape, Type eltTy, int kWidth, int opIdx) const {
  auto rep = getRepForOperands(shape, eltTy, kWidth, opIdx);
  return product(rep) * kWidth;
}

SmallVector<int64_t> HCUWmmaEncodingAttr::getElemsPerInstrForOperands() const {
  return {16, 16};
}

SmallVector<int64_t>
HCUWmmaEncodingAttr::getRepForOperands(ArrayRef<int64_t> operandShape,
...
SmallVector<unsigned> HCUWmmaEncodingAttr::getMNKDimPerInstr() {
