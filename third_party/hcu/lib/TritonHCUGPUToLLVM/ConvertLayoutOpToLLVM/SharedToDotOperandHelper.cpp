...
computeOffsetsAType(ConversionPatternRewriter &rewriter, Location loc,
  ...
  int vectorSize = 1;
  if (!interleave && srcLayout.getOrder()[0] == rank - 1) {
  ...
  auto mapping = fn(rewriter, loc, elemsPerInstr, warpId, laneId, numOfElems,
                    reps, offsets, vectorSize, nonKDim, kDim, interleave);
...
computeOffsetsBType(ConversionPatternRewriter &rewriter, Location loc,
  ...
  int vectorSize = 1;
  if (!interleave && srcLayout.getOrder()[0] == rank - 2) {
    if (isSwizzled(srcLayout))
      vectorSize = std::min(static_cast<int>(srcLayout.getVec()), numOfElems);
    else
      vectorSize = numOfElems;
  ...
  auto mapping = fn(rewriter, loc, tElemsPerInstr, warpId, laneId, numOfElems,
                    tReps, tOffsets, vectorSize, nonKDim, kDim, interleave);
