...
namespace mlir {
namespace triton {
#define GEN_PASS_DEF_DECOMPOSEUNSUPPORTEDHCUCONVERSIONS
...
struct DecomposeUnsupportedHCUConversions
    : public mlir::triton::impl::DecomposeUnsupportedHCUConversionsBase<
          DecomposeUnsupportedHCUConversions> {
  explicit DecomposeUnsupportedHCUConversions(StringRef targetArch) {
    this->arch = targetArch.str();
  }

  void runOnOperation() override {
    triton::HCU::TargetInfo targetInfo(this->arch.getValue());
    ...
    mod.walk([&](triton::gpu::ConvertLayoutOp cvtOp) -> void {
      ...
      auto srcWmma =
          dyn_cast<triton::gpu::HCUWmmaEncodingAttr>(srcType.getEncoding());
      auto dstDotOp =
    ...
    mod.walk([&](triton::gpu::ConvertLayoutOp cvtOp) -> void {
      ...
      if (!isa<triton::gpu::HCUMfmaEncodingAttr>(srcEnc) || !dstBlocked) {
        return;
      }

      auto currLDSUsage = triton::HCU::getCvtOpLDSUsage(cvtOp);
      ...
      for (int i = 0; i < factorizedNumWarps.size(); i++) {
        ...
        tmpLayouts.push_back(
            mlir::triton::HCU::createTmpLayout(srcEnc, warpsPerCTA));
      }
      ...
      auto replacementCvts = mlir::triton::HCU::createNewConvertOps(
...
createDecomposeUnsupportedConversionsPass(StringRef targetArch) {
  return std::make_unique<DecomposeUnsupportedHCUConversions>(targetArch);
}
