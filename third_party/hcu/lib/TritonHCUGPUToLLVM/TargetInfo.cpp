...
Value TargetInfo::getClusterCTAId(RewriterBase &rewriter, Location loc) const {
  // On HCU hardware we don't have CTA clusters like NVIDIA. So this will always
...
void TargetInfo::storeMatrixShared(RewriterBase &rewriter, Location loc,
  ...
  llvm::report_fatal_error("HCUGPU does not support stmatrix");
}
...
Value TargetInfo::shuffleXor(RewriterBase &rewriter, Location loc, Value val,
                             int i) const {
  return LLVM::HCU::shuffleXor(loc, rewriter, val, i);
}

Value TargetInfo::shuffleUp(RewriterBase &rewriter, Location loc, Value val,
                            int i) const {
  return LLVM::HCU::shuffleUp(loc, rewriter, val, i);
}

Value TargetInfo::shuffleIdx(RewriterBase &rewriter, Location loc, Value val,
                             int i) const {
  return LLVM::HCU::shuffleIdx(loc, rewriter, val, i);
}
