...
struct BufferEmitterr {
  ...
  BufferEmitterr(RewriterBase &rw, Location loc, HCU::TargetInfo ti)
  ...
  void emitMaskedBufferLoadLds(Type type, Value basePtr, Value offset,
    ...
    Type bufferType = getBufferOpType(vecTy);
    auto rsrc = args[0];
    auto maskedOffset = args[1];
    auto sgprOffset = args[2];
    auto cacheModifiers = args[3];
    // rewriter.create<ROCDL::RawPtrBufferLoadOp>(
    //     loc, bufferType, args, ArrayRef<NamedAttribute>());

    // required s_mov m0, $ldsBase
    {
      GCNBuilder builder;
      auto bufferLoad = *builder.create("");
      if (numBits == 128)
        bufferLoad = *builder.create("buffer_load_dwordx4");
      else if (numBits == 64)
        bufferLoad = *builder.create("buffer_load_dwordx2");
      else if (numBits == 32)
        bufferLoad = *builder.create("buffer_load_dword");
      else if (numBits == 16)
      ...
      auto offset12 = builder.newModifier("offset", "0");
...
private:
  ...
  Type getBufferOpType(VectorType vecTy) {
    ...
    Type bufferType = bufferElementType;
  ...
  void fillBufferArgs(VectorType vecTy, Value basePtr, Value offset, Value pred,
    ...
    if (targetInfo.getISAFamily() == HCU::ISAFamily::RDNA2 ||
        targetInfo.getISAFamily() == HCU::ISAFamily::RDNA3) {
      flags |= (1 << 24);
      uint32_t oob = 3;
      flags |= (oob << 28);
    }
...
Value llLoad(RewriterBase &rewriter, Location loc, Value ptr, Type elemTy,
  ...
  if (alignmentBytes &&
    ...
    // llvm::dbgs()<<"alignmentBytes"<<alignmentBytes<<"vecSize"<<vecSize<<"\n";
