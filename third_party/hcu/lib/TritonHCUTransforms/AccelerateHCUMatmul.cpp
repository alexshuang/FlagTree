...
warpsPerTileWMMA(tt::DotOp dotOp, const ArrayRef<int64_t> shape, int numWarps) {
  return warpsPerTile(dotOp, shape, numWarps,
                      {ttg::HCUWmmaEncodingAttr::getMNKDimPerInstr()[0],
                       ttg::HCUWmmaEncodingAttr::getMNKDimPerInstr()[1]});
}
...
public:
  ...
  FailureOr<MfmaInsn> chooseMfmaInstructionV2(tt::DotOp dot) const {
  ...
  /// @brief Choose MFMA instruction parameters for mmacfuse 16x64x32
  /// @param dot target dot operation
  /// @return MfmaInsn or failure
  FailureOr<MfmaInsn> chooseMfmaInstructionV3(tt::DotOp dot) const {
    // number of matrix elements along k dim per one MFMA intruction
    unsigned kDim = 0;
    auto opType = cast<RankedTensorType>(dot.getA().getType());
    auto dataTypeA = opType.getElementType();
  ...
  /// @param dot target dot operation
  /// @return MfmaInsn or failure
  FailureOr<MfmaInsn> chooseMfmaInstructionV4(tt::DotOp dot) const {
    // number of matrix elements along k dim per one MFMA intruction
    unsigned kDim = 0;
    auto opType = cast<RankedTensorType>(dot.getA().getType());
    auto dataTypeA = opType.getElementType();
  ...
  /// @param dot target dot operation
  /// @return MfmaInsn or failure
  FailureOr<MfmaInsn> chooseMfmaInstructionV5(tt::DotOp dot) const {
    // number of matrix elements along k dim per one MFMA intruction
    unsigned kDim = 0;
    auto opType = cast<RankedTensorType>(dot.getA().getType());
    auto dataTypeA = opType.getElementType();
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    ttg::HCUMfmaEncodingAttr mfmaEnc;
    ...
    mfmaEnc = ttg::HCUMfmaEncodingAttr::get(
        ...
        /*instrShape*/ mDim, nDim, kDim, isTransposed, interleave, CTALayout);
...
public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    // check shape
    auto mnkDim = ttg::HCUWmmaEncodingAttr::getMNKDimPerInstr();
    ...
    wmmaEnc = ttg::HCUWmmaEncodingAttr::get(ctx, wmmaVersion, warpsPerTile,
                                            CTALayout);
...
class TritonHCUGPUAccelerateMatmulPass
    : public TritonHCUGPUAccelerateMatmulBase<
          TritonHCUGPUAccelerateMatmulPass> {
public:
  ...
  TritonHCUGPUAccelerateMatmulPass(StringRef archGen, int matrixInstructionSize,
    ...
    this->kPack = kPack;
    this->num_ldmatrixes = num_ldmatrixes;
    this->enable_mmacfuse = enable_mmacfuse;
  }
