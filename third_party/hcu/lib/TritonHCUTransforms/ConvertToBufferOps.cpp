...
struct ConvertTritonLoadToBufferLoad
  ...
  matchAndRewrite(triton::LoadOp op, PatternRewriter &rewriter) const override {
    ...
    if (canUseBufferOps(ptr, assumptions)) {
      ...
      rewriter.replaceOpWithNewOp<triton::hcugpu::BufferLoadOp>(
...
class TritonHCUGPUConvertToBufferOpsPass
    : public TritonHCUGPUConvertToBufferOpsBase<
          TritonHCUGPUConvertToBufferOpsPass> {

public:
  TritonHCUGPUConvertToBufferOpsPass() = default;
...
std::unique_ptr<Pass> mlir::createTritonHCUGPUConvertToBufferOpsPass() {
  return std::make_unique<TritonHCUGPUConvertToBufferOpsPass>();
}
