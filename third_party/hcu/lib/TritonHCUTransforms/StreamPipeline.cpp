...
void LoopPipeliner::emitEpilogue(DenseMap<Value, Value> &newResults) {
  ...
  unsigned int bw = 32;
  auto ubType = ub.getType();
  if (auto intType = dyn_cast<mlir::IntegerType>(ubType)) {
    bw = intType.getWidth();
  }
  Value one = builder.create<arith::ConstantIntOp>(loc, 1, bw);
...
// Stream Pipeline
struct PipelinePass : public TritonHCUGPUStreamPipelineBase<PipelinePass> {
...
std::unique_ptr<Pass> mlir::createTritonHCUGPUStreamPipelinePass() {
