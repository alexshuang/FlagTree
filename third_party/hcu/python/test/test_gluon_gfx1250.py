...
def gemm_kernel(a_ptr, b_ptr, c_ptr,  #
    ...
    WMMA_LAYOUT: ttgl.constexpr = ttgl.hcu.AMDWMMALayout(3, True, [2, 2], [16, 16, INSTR_SHAPE_K])
...
def gemm_async_pipelined_kernel(a_ptr, b_ptr, c_ptr,  #
    ...
    BLOCKED_LAYOUT: ttgl.constexpr = ttgl.BlockedLayout([1, 8], [4, 8], [4, 1], [1, 0])
    WMMA_LAYOUT: ttgl.constexpr = ttgl.hcu.AMDWMMALayout(3, True, [2, 2], [16, 16, 32])
    ...
    for _ in ttgl.static_range(NUM_BUFFERS - 1):
        if USE_TDM:
            ttgl.hcu.gfx1250.tdm.async_load(a_desc, [0, load_idx * BLOCK_K],  #
                                            a_buffer.index(load_idx % NUM_BUFFERS))
            ttgl.hcu.gfx1250.tdm.async_load(b_desc, [load_idx * BLOCK_K, 0],  #
                                            b_buffer.index(load_idx % NUM_BUFFERS))
        else:
            mask_a = offs_ak[None, :] < K - load_idx * BLOCK_K
            ttgl.hcu.gfx1250.async_copy.global_to_shared(a_buffer.index(load_idx % NUM_BUFFERS), a_ptrs, mask_a,
                                                         other=0.0)

            mask_b = offs_bk[:, None] < K - load_idx * BLOCK_K
            ttgl.hcu.gfx1250.async_copy.global_to_shared(b_buffer.index(load_idx % NUM_BUFFERS), b_ptrs, mask_b,
                                                         other=0.0)
            ttgl.hcu.gfx1250.async_copy.commit_group()
    ...
    for _ in range(0, ttgl.cdiv(K, BLOCK_K) - (NUM_BUFFERS - 1)):
        ...
        if USE_TDM:
            ttgl.hcu.gfx1250.tdm.async_wait((NUM_BUFFERS - 1) * 2)
        else:
            ttgl.hcu.gfx1250.async_copy.wait_group((NUM_BUFFERS - 1))
    ...
    for i in ttgl.static_range(NUM_BUFFERS - 1):
        if USE_TDM:
            ttgl.hcu.gfx1250.tdm.async_wait((NUM_BUFFERS - 2 - i) * 2)
        else:
            ttgl.hcu.gfx1250.async_copy.wait_group((NUM_BUFFERS - 2 - i))
...
def gemm_async_kernel(a_ptr, b_ptr, c_ptr,  #
    ...
    # Descriptors for TDM
    a_desc = ttgl.hcu.gfx1250.tdm.make_tensor_descriptor(base=a_ptr + pid_m * BLOCK_M * stride_am, shape=(M, K),
                                                         strides=(stride_am, stride_ak), block_shape=(BLOCK_M, BLOCK_K),
                                                         layout=SHARED_LAYOUT_A)
    b_desc = ttgl.hcu.gfx1250.tdm.make_tensor_descriptor(base=b_ptr + pid_n * BLOCK_N * stride_bn, shape=(K, N),
    ...
    for k in range(0, ttgl.cdiv(K, BLOCK_K)):
        if USE_TDM:
            ttgl.hcu.gfx1250.tdm.async_load(a_desc, [0, k * BLOCK_K], a_buffer)
            ttgl.hcu.gfx1250.tdm.async_load(b_desc, [k * BLOCK_K, 0], b_buffer)
            ttgl.hcu.gfx1250.tdm.async_wait(0)
        else:
            mask_a = offs_ak[None, :] < K - k * BLOCK_K
            ttgl.hcu.gfx1250.async_copy.global_to_shared(a_buffer, a_ptrs, mask_a, other=0.0)

            mask_b = offs_bk[:, None] < K - k * BLOCK_K
            ttgl.hcu.gfx1250.async_copy.global_to_shared(b_buffer, b_ptrs, mask_b, other=0.0)
            ttgl.hcu.gfx1250.async_copy.commit_group()
            a_ptrs += BLOCK_K * stride_ak
            b_ptrs += BLOCK_K * stride_bk
            ttgl.hcu.gfx1250.async_copy.wait_group(0)
...
def test_hcu_wmma_scaled(M, N, K, a_type, b_type):
    ...
    class Layout:
        ...
        def __init__(self, a_type, b_type, scale_nonk, scale_k):
            ...
            wmma_layout = ttgl.hcu.AMDWMMALayout(version=3, transposed=True, warps_per_cta=[2, 2],
                                                 instr_shape=[16, 16, 128])
            wmma_layout_packed = ttgl.hcu.AMDWMMALayout(version=3, transposed=True, warps_per_cta=[2, 2],
    ...
    def kernel(c_ptr, a_ptr, a_scale_ptr, b_ptr, b_scale_ptr,  #
        ...
        c = ttgl.hcu.gfx1250.wmma_scaled(a, a_scale, a_type, b, b_scale, b_type, zero)
...
def test_hcu_wmma_scaled_tdm(M, N, K, mxfp_type, hasScale):
    ...
    def scaled_wmma_tdm_gluon_kernel(a_base, stride_am, stride_ak, a_scale, b_base, stride_bk, stride_bn, b_scale, out,
        ...
        wmma_layout: ttgl.constexpr = ttgl.hcu.AMDWMMALayout(version=3, transposed=True, warps_per_cta=[2, 2],
                                                             instr_shape=[16, 16, 128])
        wmma_layout_packed: ttgl.constexpr = ttgl.hcu.AMDWMMALayout(version=3, transposed=True, warps_per_cta=[2, 2],
                                                                    instr_shape=[16, 16, 64])

        zero = ttgl.zeros([BLOCK_M, BLOCK_N], dtype=ttgl.float32, layout=wmma_layout)

        a_desc = ttgl.hcu.gfx1250.tdm.make_tensor_descriptor(base=a_base, shape=(BLOCK_M, PACKED_BLOCK_K_A),
                                                             strides=(stride_am, stride_ak),
                                                             block_shape=(BLOCK_M, PACKED_BLOCK_K_A),
                                                             layout=SHARED_LAYOUT_A)
        a_buffer = ttgl.allocate_shared_memory(a_desc.dtype, shape=a_desc.block_shape, layout=a_desc.layout)
        ttgl.hcu.gfx1250.tdm.async_load(a_desc, [0, 0], a_buffer)
        ttgl.hcu.gfx1250.tdm.async_wait(0)
        ...
        b_desc = ttgl.hcu.gfx1250.tdm.make_tensor_descriptor(base=b_base, shape=(PACKED_BLOCK_K_B, BLOCK_N),
                                                             strides=(stride_bk, stride_bn),
                                                             block_shape=(PACKED_BLOCK_K_B, BLOCK_N),
                                                             layout=SHARED_LAYOUT_B)
        b_buffer = ttgl.allocate_shared_memory(b_desc.dtype, shape=b_desc.block_shape, layout=b_desc.layout)
        ttgl.hcu.gfx1250.tdm.async_load(b_desc, [0, 0], b_buffer)
        ttgl.hcu.gfx1250.tdm.async_wait(0)
        ...
        c = ttgl.hcu.gfx1250.wmma_scaled(a, scale_a, type_a, b, scale_b, type_b, zero)
...
def tensor_async_copy_kernel(a_ptr, b_ptr, M, N,  #
    ...
    for i in ttgl.static_range(0, NUM_BUFFERS):
        ...
        ttgl.hcu.gfx1250.async_copy.global_to_shared(a_buffer.index(i), a_ptrs, a_mask, other=0.0)
        ttgl.hcu.gfx1250.async_copy.commit_group()
...
def tensor_device_tdm_copy_kernel(a_ptr, b_ptr, M, N,  #
    ...
    a_desc = ttgl.hcu.gfx1250.tdm.make_tensor_descriptor(base=a_ptr, shape=(M, N), strides=(N, 1),
    ...
    for i in ttgl.static_range(0, NUM_BUFFERS):
        idx_n = pid_n * (BLOCK_N * NUM_BUFFERS) + i * BLOCK_N
        ttgl.hcu.gfx1250.tdm.async_load(a_desc, [idx_m, idx_n], a_buffer.index(i))
...
def test_runtime_tensor_copy(M, N, BLOCK_M, BLOCK_N, NUM_BUFFERS, ASYNC_LOAD_TYPE, NUM_WARPS):
    ...
    if ASYNC_LOAD_TYPE == "ASYNC_COPY":
        ...
        assert ASYNC_LOAD_TYPE == "HOST_TDM"
        smem_layout = ttgl.PaddedSharedLayout.with_identity_for([[32, 4]], [BLOCK_M, BLOCK_N], [1, 0])
        a_desc = gluon.hcu.gfx1250.TensorDescriptor.from_tensor(a_device, [BLOCK_M, BLOCK_N], layout=smem_layout)
...
def tensor_fill_kernel(a_ptr, M, N, BLOCK_M: ttgl.constexpr, BLOCK_N: ttgl.constexpr, NUM_BUFFERS: ttgl.constexpr):
    ...
    for i in ttgl.static_range(0, NUM_BUFFERS):
        idx_n = pid_n * (BLOCK_N * NUM_BUFFERS) + i * BLOCK_N
        ttgl.hcu.gfx1250.tdm.async_store(a_desc, [idx_m, idx_n], a_buffer.index(i))
...
def tensor_descriptor_load_store_nd_kernel(out_ptr, a_ptr, shape, strides, BLOCK_SHAPE, out_shape, out_strides,
    ...
    desc = ttgl.hcu.gfx1250.tdm.make_tensor_descriptor(base=a_ptr, shape=shape, strides=strides,
    ...
    ttgl.hcu.gfx1250.tdm.async_load(desc, offs, block_shared)
    ttgl.hcu.gfx1250.tdm.async_wait(0)

    out_desc = ttgl.hcu.gfx1250.tdm.make_tensor_descriptor(base=out_ptr, shape=out_shape, strides=out_strides,
                                                           block_shape=BLOCK_SHAPE, layout=SHARED_LAYOUT)

    ttgl.hcu.gfx1250.tdm.async_store(out_desc, offs, block_shared)
    ttgl.hcu.gfx1250.tdm.async_wait(0)
...
def mxgemm_kernel(a_ptr, b_ptr, c_ptr, a_scale, b_scale, M, N, K, stride_am, stride_ak, stride_bk, stride_bn, stride_cm,
    ...
    WMMA_LAYOUT: ttgl.constexpr = ttgl.hcu.AMDWMMALayout(3, transposed=True, warps_per_cta=[2, 2],
                                                         instr_shape=[16, 16, 128])
    WMMA_LAYOUT_PACKED: ttgl.constexpr = ttgl.hcu.AMDWMMALayout(3, transposed=True, warps_per_cta=[2, 2],
                                                                instr_shape=[16, 16, 64])
    ...
    A_SCALE_LINEAR_LAYOUT: ttgl.constexpr = ttgl.hcu.gfx1250.get_wmma_scale_layout(DOT_LAYOUT_A,
                                                                                   [BLOCK_M, BLOCK_K_SCALE])
    B_SCALE_LINEAR_LAYOUT: ttgl.constexpr = ttgl.hcu.gfx1250.get_wmma_scale_layout(DOT_LAYOUT_B,
    ...
    for k in range(0, ttgl.cdiv(K, BLOCK_K)):
        ...
        accumulator = ttgl.hcu.gfx1250.wmma_scaled(a, scale_a, DTYPE_A, b, scale_b, DTYPE_B, accumulator)
...
def test_compile_mxgemm(BLOCK_M, BLOCK_N, BLOCK_K, DTYPE_A, DTYPE_B):
    ...
    assert re.search(pattern, amdgcn), f"Can't find instruction {pattern} in HCUGCN assembly"
...
def async_load_and_write_back_kernel(a_ptr, out_ptr, M, N, BLOCK_M: ttgl.constexpr, BLOCK_N: ttgl.constexpr,
    ...
    ttgl.hcu.gfx1250.async_copy.global_to_shared(buffer, a_ptrs)
    ttgl.hcu.gfx1250.async_copy.commit_group()
    ttgl.hcu.gfx1250.async_copy.wait_group(0)
...
def scaled_wmma_scale_preshuffle(a_base, stride_am, stride_ak, a_scale, b_base, stride_bk, stride_bn, b_scale, out,
    ...
    wmma_layout: ttgl.constexpr = ttgl.hcu.AMDWMMALayout(version=3, transposed=TRANSPOSED_WMMA, warps_per_cta=[2, 2],
                                                         instr_shape=[16, 16, 128], tiles_per_warp=tiles_per_warp)
    wmma_layout_packed: ttgl.constexpr = ttgl.hcu.AMDWMMALayout(version=3, transposed=TRANSPOSED_WMMA,
    ...
    a_scale_linear_layout: ttgl.constexpr = ttgl.hcu.gfx1250.get_wmma_scale_layout(operand_a_layout,
                                                                                   [BLOCK_M, SCALE_BLOCK_K])
    b_scale_linear_layout: ttgl.constexpr = ttgl.hcu.gfx1250.get_wmma_scale_layout(operand_b_layout,
...
def test_compile_wmma_scale_preshuffle(M, N, K, type_a, type_b, TRANSPOSED_WMMA):
    ...
    for suffix in (scale_opsel_a, scale_opsel_b, f"{scale_opsel_a} {scale_opsel_b}"):
        ...
        assert re.search(pattern, amdgcn), f"Can't find pattern {pattern} in HCUGCN assembly"
...
def async_copy_mbarrier_kernel(a_ptr, out_ptr, M, N, BLOCK_M: ttgl.constexpr, BLOCK_N: ttgl.constexpr,
    ...
    mbar = ttgl.allocate_shared_memory(ttgl.int64, [1], ttgl.hcu.gfx1250.mbarrier.MBarrierLayout())
    buffer = ttgl.allocate_shared_memory(a_ptr.type.element_ty, [BLOCK_M, BLOCK_N], shared_layout)
    # NOTE: Setting count = NUM_WARPS * WARP_SIZE * 2 is only for testing purposes, in order to also exercise the ttgl.hcu.gfx1250.mbarrier.arrive API.
    # In practice, since we know that phase is initialized to 0, we can just set count = NUM_WARPS * WARP_SIZE and call directly ttgl.hcu.gfx1250.mbarrier.wait(mbar, 0).
    ttgl.hcu.gfx1250.mbarrier.init(mbar, count=NUM_WARPS * WARP_SIZE * 2)
    ttgl.hcu.gfx1250.async_copy.global_to_shared(buffer, a_ptrs)
    ttgl.hcu.gfx1250.async_copy.mbarrier_arrive(mbar)
    prior_phase = ttgl.hcu.gfx1250.mbarrier.arrive(mbar)
    ttgl.hcu.gfx1250.mbarrier.wait(mbar, prior_phase)
...
def tensor_async_copy_mbarrier_kernel(a_ptr, b_ptr, M, N,  #
    ...
    a_desc = ttgl.hcu.gfx1250.tdm.make_tensor_descriptor(base=a_ptr, shape=(M, N), strides=(N, 1),
                                                         block_shape=(BLOCK_M, BLOCK_N), layout=SHARED_LAYOUT)
    bars = ttgl.allocate_shared_memory(ttgl.int64, [NUM_BUFFERS, 1], ttgl.hcu.gfx1250.mbarrier.MBarrierLayout())
    a_buffer = ttgl.allocate_shared_memory(a_desc.dtype, [NUM_BUFFERS] + a_desc.block_shape, a_desc.layout)
    ...
    # NOTE: Setting count = NUM_WARPS + NUM_WARPS * WARP_SIZE is only for testing purposes, in order to also exercise the ttgl.hcu.gfx1250.mbarrier.arrive API.
    # In practice, since we know that phase is initialized to 0, we can just set count = NUM_WARPS and call directly ttgl.hcu.gfx1250.mbarrier.wait(bars.index(i), 0).

    for i in ttgl.static_range(0, NUM_BUFFERS):
        ttgl.hcu.gfx1250.mbarrier.init(bars.index(i), count=NUM_WARPS + NUM_WARPS * WARP_SIZE)

    idx_m = pid_m * BLOCK_M
    for i in ttgl.static_range(0, NUM_BUFFERS):
        idx_n = pid_n * (BLOCK_N * NUM_BUFFERS) + i * BLOCK_N
        ttgl.hcu.gfx1250.tdm.async_load(a_desc, [idx_m, idx_n], a_buffer.index(i), mbarrier=bars.index(i))

    for i in ttgl.static_range(0, NUM_BUFFERS):
        prior_phase = ttgl.hcu.gfx1250.mbarrier.arrive(bars.index(i))
        ttgl.hcu.gfx1250.mbarrier.wait(bars.index(i), prior_phase)
        idx_n = pid_n * (BLOCK_N * NUM_BUFFERS) + i * BLOCK_N
...
def test_tdm_load_pred():
    ...
    def kernel(a_ptr, b_ptr):
        ...
        desc = ttgl.hcu.gfx1250.tdm.make_tensor_descriptor(base=a_ptr, shape=(16, 64), strides=(64, 1),
        ...
        ttgl.hcu.gfx1250.tdm.async_load(desc, [0, 0], smem, pred=False)
        ttgl.hcu.gfx1250.tdm.async_wait(0)
        tile1 = smem.load(reg_layout)
        ttgl.store(b_ptrs, tile1)

        ttgl.hcu.gfx1250.tdm.async_load(desc, [0, 32], smem, pred=True)
        ttgl.hcu.gfx1250.tdm.async_wait(0)
