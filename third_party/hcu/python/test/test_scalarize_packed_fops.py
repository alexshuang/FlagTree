...
def get_func_body(llir):
    func_body = re.findall(r"define hcugpu_kernel void .*? \{(.* ret void.*?)}", llir, flags=re.DOTALL)
...
def test_check_not_scalarize():
    triton.knobs.hcu.scalarize_packed_fops = False
...
def test_check_scalarized():
    triton.knobs.hcu.scalarize_packed_fops = True
    kernel = triton.compile(str(Path(__file__).parent / "attn_fwd.ttir"), target=current_target)
