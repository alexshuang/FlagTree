...
def dot(lhs: tl.tensor, rhs: tl.tensor, acc: tl.tensor, input_precision: Optional[str], max_num_imprecise_acc: int,
    ...
    num_ldmatrixes = builder.options.num_ldmatrixes
    if num_ldmatrixes == 2:
        assert lhs.shape[-2].value >= min_dot_size[0] * 2 and lhs.shape[-1].value >= min_dot_size[2] \
        and rhs.shape[-1].value >= min_dot_size[1] * 2, \
            f"Input shapes should have M >= {min_dot_size[0] * 2}, N >= {min_dot_size[1] * 2} and K >= {min_dot_size[2]}"
    if num_ldmatrixes == 1:
        assert lhs.shape[-2].value >= min_dot_size[0] and lhs.shape[-1].value >= min_dot_size[2] \
        and rhs.shape[-1].value >= min_dot_size[1] * 2, \
    ...
    if enable_mmacfuse == 1:
        ...
        assert lhs.shape[-2].value >= min_dot_size[0] and lhs.shape[-1].value >= min_dot_size[2]*2 \
        and rhs.shape[-1].value >= min_dot_size[1] * 4, \
            f"Input shapes should have M >= {min_dot_size[0]}, N >= {min_dot_size[1] * 4} and K >= {min_dot_size[2]}"
